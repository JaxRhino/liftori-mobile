...
2026-04-18 — **One `rep_handle` column powers N QR surfaces — don't duplicate the claim flow.** Wave 13 (My QR → `/meet/:handle`) and Wave 14 (Investor QR → `/invest/:handle`) both resolve the same `profiles.rep_handle` via `resolve_rep_handle()`. The mobile investor screen does NOT ship its own claim UI — its empty state redirects to `/my-qr` which is the single source of truth for picking/editing the handle. Rule of thumb: if a new QR surface points at the same rep, reuse the handle; if it points at a different entity (team, project, org), give it its own column. Reference: `src/lib/qrService.ts:getMyHandle`, `src/lib/investorQrService.ts:investUrlForHandle`, `app/(app)/investor-qr.tsx` empty-state action.

2026-04-18 — **Anon-submittable forms = SECURITY DEFINER RPC + explicit GRANTs, not direct table INSERT.** `/invest/:handle` is a public page — investors submit without an auth session. Exposing `investor_leads` for anon INSERT would mean writing RLS policies that let strangers touch the raw table. Instead: keep the table founder-only (RLS), and expose a single `submit_investor_lead(12 params) SECURITY DEFINER` RPC that validates input + writes the row. Grant `EXECUTE` on the function to `anon` explicitly. Same pattern for `investor_qr_scans` INSERT — anon can write only via the RPC path. Per `feedback_postgrest_grants`, new tables also need `GRANT` to `authenticated`/`anon` beyond RLS; for tables with bigint identity columns, grant the sequence too.

2026-04-18 — **Confirm RPC return shape before binding field names in the UI.** First pass at the Investors admin page assumed `investor_pipeline_summary()` returned `{scans_30d, scans_all, leads_30d, leads_all, meetings_scheduled, meetings_completed, committed_cents_all}` — wrong. Actual keys: `{leads_total, leads_30d, scans_30d, scheduled, deck_sent, committed, committed_cents}`. Rule: before rendering any RPC-backed dashboard, run `SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname='…'` and read the `json_build_object(…)` body — it's the ground truth for the keys you'll destructure. The `my_investor_qr_stats` mobile-facing RPC uses a different (more verbose) shape on purpose — each RPC picks its own key names; don't assume consistency across the API.

2026-04-18 — **Founder-only surfaces: gate BOTH the nav item and the page body.** Pattern used for `Investors`: (a) add label to `FOUNDER_ONLY_ITEMS` in `AdminLayout.jsx` so non-founders never see the nav, AND (b) check `isFounder({ email, personal_email })` at the top of the page component and render a "founder only" message if false. Belt + suspenders. Someone linking direct to `/admin/investors` still hits the page body's guard. RLS at the table level is the third layer; if the other two fail, Postgres still refuses the query. Keep the client-side `testerProgramService.LIFTORI_FOUNDERS` email list in sync with the SQL `is_founder()` helper — they are two sources of truth for the same rule.
